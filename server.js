...
app.post("/api/account/email/start", async (req, res) => {
  const { telegramId, email, role } = req.body; // role: "buyer" | "artist"
  ...
  await upsertAccount({
    ...
    verificationExpiresAt: Date.now() + 10 * 60 * 1000, // 10 daqiqa amal qiladi
  });
...
app.post("/api/account/email/confirm", async (req, res) => {
 ...
 * ------------------------------------------------------------- */
async function getProduct(id) { /* DB lookup */ }
async function getOrder(id) { /* DB lookup */ }
async function markOrderPaid(id) { /* DB update */ }
...
function requireAuth(req, res, next) { /* verify Telegram initData */ next(); }

function isValidEmail(email) { return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email || ""); }
async function getAccountByTelegramId(telegramId) { /* DB lookup */ }
async function createProduct(fields) { /* DB insert, returns created product */ }
async function upsertAccount(fields) { /* DB insert/update by telegramId */ }
async function setArtistSubscriptionActive(telegramId, active, extra) { /* DB update, extra e.g. { subscriptionExpiresAt } */ }
async function getArtistsExpiringWithinDays(days) { /* DB lookup */ return []; }
async function getArtistsWithExpiredSubscription() { /* DB lookup */ return []; }
async function recordLedgerEntry(fields) { /* DB insert */ }
async function getLedgerEntriesForArtist(telegramId) { /* DB lookup, returns array */ return []; }
async function markMaturedEntriesWithdrawable() { /* DB: WHERE receivedAt < now-21d SET status=withdrawable */ }
async function getArtistsWithWithdrawableBalance() { /* DB lookup */ return []; }
...
async function markArtistEntriesPaidOut(telegramId) { /* DB update */ }
async function notifyArtist(telegramId, message) { /* send Telegram message via bot API */ }
async function recordReferral(fields) { /* DB insert */ }
